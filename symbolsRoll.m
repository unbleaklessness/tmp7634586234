function [M, C, Cs, Cx, G, Gd] = symbolsRoll()
    syms thet thetD thetDD real; % Pendulum base angle (around X axis).
    ...
    q = [thet; alph1; gam];
    qD = [thetD; alph1D; gamD];
    qDD = [thetDD; alph1DD; gamDD];
    ...
    % Bodies rotations:
    R(:, :, 1) = simplify(Ry(thet));
    R(:, :, 2) = simplify(R(:, :, 1) * Rz(pi) * Rx(alph1));
    R(:, :, 3) = simplify(R(:, :, 2) * Rz(-gam));
    R(:, :, 4) = simplify(R(:, :, 1) * Rz(0) * Rx(alph1));
