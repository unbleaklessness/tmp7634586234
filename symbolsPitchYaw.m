function [M, C, Cs, Cx, G, Gd, AMd, AMTd, artefacts] = symbolsPitchYaw()
    syms psih psihD psihDD real; % Pendulum base angle (around Z axis).
    syms thet thetD thetDD real; % Pendulum base angle (around Y axis).
    syms alph1 alph1D alph1DD real; % Actuator angle.
    syms alph2 alph2D alph2DD real; % Actuator angle.
    syms gam gamD gamDD real; % Disc angle.
    syms nu1 real; % Actuator zero position;
    syms nu2 real; % Actuator zero position;
    ...
    syms lean real; % Leaning on one leg or another.

    % Generalized coordiantes:
    q = [psih; thet; alph1; alph2; gam];
    qD = [psihD; thetD; alph1D; alph2D; gamD];
    qDD = [psihDD; thetDD; alph1DD; alph2DD; gamDD];
    ...
    % Bodies rotations:
    R(:, :, 1) = simplify(Rz(psih) * Ry(thet));
    R(:, :, 2) = simplify(R(:, :, 1) * Rz(pi) * Rx(nu1) * Rx(alph1));
    R(:, :, 3) = simplify(R(:, :, 2) * Rz(-gam));
    R(:, :, 4) = simplify(R(:, :, 1) * Rz(0) * Rx(nu2) * Rx(alph2));
    R(:, :, 5) = simplify(R(:, :, 4) * Rz(gam));
    R(:, :, 6) = simplify(R(:, :, 1) * Rz(3 * pi / 2));
    R(:, :, 7) = simplify(R(:, :, 6) * Rz(gam));
    R(:, :, 8) = simplify(R(:, :, 1) * Rz(pi / 2));
    ...
    % Bodies center of mass locations:
    L(:, 1) = R(:, :, 1) * [0; 0 - lean; c1z];
    L(:, 2) = R(:, :, 1) * [0; c2y - lean; c2z];
    L(:, 3) = R(:, :, 1) * [0; c2y - lean; c2z];
    L(:, 4) = R(:, :, 1) * [0; -c2y - lean; c2z];
    L(:, 5) = R(:, :, 1) * [0; -c2y - lean; c2z];
    L(:, 6) = R(:, :, 1) * [-c6x; 0 - lean; c6z];
    L(:, 7) = R(:, :, 1) * [-c6x; 0 - lean; c6z];
    L(:, 8) = R(:, :, 1) * [c6x; 0 - lean; c6z];
    L(:, 9) = R(:, :, 1) * [c6x; 0 - lean; c6z];
    ...
    % Lagrangian:
    La = T - U;
    LEquations0 = Lagrange(La, reshape([q qD qDD]', 1, size(q, 1) + size(qD, 1) + size(qDD, 1)));
    ...
    AM = simplify(diff(La, psihD));
    AMT = simplify(diff(La, thetD));
    disp('Angular momentum calculated.');

    assume(gamDD == 0);
    q = [psih; thet; alph1; alph2];
    qD = [psihD; thetD; alph1D; alph2D];
    qDD = [psihDD; thetDD; alph1DD; alph2DD];
    ...
    AMd = sym(zeros(1, size(qI, 1)));
    for a = 1:size(qI, 1)
        AMd(:, a) = simplify(diff(AM, qI(a)));
    end

    AMTd = sym(zeros(1, size(qI, 1)));
    for a = 1:size(qI, 1)
        AMTd(:, a) = simplify(diff(AMT, qI(a)));
    end
    ...
    artefacts.La = La;
    artefacts.U = U;
    artefacts.TLinear = TLinear;
    artefacts.TRotational = TRotational;
    artefacts.L = L;
    artefacts.v = v;
    artefacts.w = w;
    artefacts.masses = masses;
end